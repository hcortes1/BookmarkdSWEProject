...
def create_goal(user_id: int,
                ...
                reminder_enabled: bool = False) -> Tuple[bool, str]:
    if goal_type not in {'pages_per_day', 'books_per_month', 'deadline'}:
        return False, "Invalid goal type"

    start_date = start_date or date.today()

    if goal_type == 'deadline' and not end_date:
        return False, "Deadline goals require an end date"
        ...
        with get_conn() as conn:
            with conn.cursor(cursor_factory=psycopg2.extras.RealDictCursor) as cur:
                cur.execute("""
                    ...
                    (user_id, target_books, progress, start_date, end_date, reminder_enabled)
                    VALUES (%s, %s, %s, %s, %s, %s)
                    RETURNING goal_id
                """, (int(user_id), int(target), 0, start_date, end_date, bool(reminder_enabled)))
            conn.commit()
...
# Wrapper to set goal progress and return success flag and message
...
# Modify the target, end date, or reminder flag of an existing goal
def modify_goal(goal_id: int,
                target_books: Optional[int] = None,
                end_date: Optional[date] = None,
                reminder_enabled: Optional[bool] = None) -> Dict[str, Any]:
    updates = []
    params = []
...
# Delete a reading goal by its ID
...
# Return a list of active goals with reminders due today or later, including progress percentage and days left
