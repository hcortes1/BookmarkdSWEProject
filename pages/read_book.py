...
from backend.rentals import check_book_rental_status
import requests
import re
...
def extract_headers_from_html(html_content):
    ...
    if toc_section:
        toc_links = toc_section.find_all(
    ...
    for header_tag in soup.find_all(['h1', 'h2', 'h3', 'h4', 'h5', 'h6']):
        ...
        for toc_text, toc_id in toc_mappings.items():
            ...
            normalized_toc = normalized_toc.rstrip(
                '.,!?')  # Remove trailing punctuation
            if normalized_header == normalized_toc:
                header_id = toc_id
                # Mark this header as having TOC mapping
                toc_header_ids.add(id(header_tag))
                break
...
def layout(book_id=None, **kwargs):
    ...
    try:
        ...
        # Return layout with content that will be conditionally shown based on rental status
        return html.Div([
            dcc.Store(id='read-book-session-check', data={'book_id': book_id}),
            html.Div(id='read-book-content', children=[
                # Content will be populated by callback after rental check
            ])
        ])
        try:
        ...
        if '<body' in html_content:
            html_content = re.sub(
                r'(<body[^>]*>)', r'\1' + nav_panel_html, html_content, count=1)
        else:
...
# Callback to check rental status and show/hide content
@callback(
    Output('read-book-content', 'children'),
    [Input('read-book-session-check', 'data'),
     Input('user-session', 'data')]
)
def check_rental_and_show_content(book_data, session_data):
    """Check if user has rental access and show content accordingly"""
    ...
    if not session_data or not session_data.get('logged_in'):
        return html.Div([
            ...
            html.P("You must be logged in to read books."),
            dcc.Link("Login", href="/login", className="blue-btn")
    ...
    if not rental_status:
        return html.Div([
            html.H3("Rental Required"),
            html.P("You need to rent this book to read it."),
            dcc.Link(
                "Rent Book", href=f"/book/{book_id}", className="blue-btn")
        ], className="error-message")

    # User has rental - show the book content
    try:
        response = requests.get(book_data['html_path'])
        response.raise_for_status()
        html_content = response.text
