...
def goal_card(goal: dict):
    ...
    # Determine target text WITHOUT goal_type
    if end_date:
        try:
            if hasattr(end_date, 'strftime'):
                days_left = (end_date - date.today()).days
                if days_left >= 0:
                    target_text = f"{target} pages ({days_left} days left)"
                else:
                    target_text = f"{target} pages (overdue)"
            else:
                target_text = f"{target} pages"
        except Exception:
            target_text = f"{target} pages"
    else:
        target_text = f"{target} pages/books"
    ...
    return html.Div([
        html.Div([
            ...
            html.Div(f"Target: {target_text}", className='goal-field'),
...
layout = html.Div([
    ...
    .goal-card {
        ...
        background-color: #dcdcdc; color: #111;
    }
    @media (prefers-color-scheme: dark) {
        .goal-card {background-color: #2a2a2a; color: #eee; box-shadow: 0 2px 6px rgba(0,0,0,0.3);}
    }
    .goal-title {font-weight: 700; font-size: 18px; margin-bottom: 4px;}
    .goal-field {font-size: 14px; margin-top: 4px;}
    .create-goal-form {padding: 16px; border-radius: 12px; box-shadow: 0 1px 6px rgba(0,0,0,0.2); background-color: #e0e0e0; color: #111;}
    @media (prefers-color-scheme: dark) {
    ...
    html.Div([
        ...
        html.Div([html.Div([
            ...
            html.Label("Target (Pages, Books, Pages By Deadline):", style={'fontWeight': 600, 'marginTop': '12px'}),
            dcc.Input(id='rg-target', type='number', placeholder='Target (required)', min=1,
        ...
        ], className='create-goal-form')], style={'marginBottom': '24px'}),
...
@callback(
    ...
    Output({'type': 'set-input', 'i': dash.ALL}, 'value'),
    Output('rg-end-date', 'date'),  # Clear date after any action
...
def handle_create_or_actions(create_clicks, set_clicks, delete_clicks,
    ...
    if not session_data or not session_data.get('logged_in'):
        return "Please log in.", dash.no_update, dash.no_update, dash.no_update

    user_id = int(session_data['user_id'])
    triggered = ctx.triggered_id
    cleared_values = [None for _ in set_values]

    if not triggered:
        return dash.no_update, dash.no_update, cleared_values, dash.no_update

    # CREATE
    if triggered == 'rg-create-btn':
        if not target:
            return "Target is required.", dash.no_update, cleared_values, dash.no_update
        if goal_type == 'deadline' and not end_date:
            return "Deadline goals require an end date.", dash.no_update, cleared_values, dash.no_update
        ...
        if res[0]:
            return "Goal created.", {"last_action": "create", "time": datetime.now().isoformat()}, cleared_values, None
        else:
            return f"Error: {res[1]}", dash.no_update, cleared_values, None
    ...
    if isinstance(triggered, dict):
        ...
        if typ == 'set-btn':
            ...
            if val_index is None:
                return "No input found for this goal.", dash.no_update, cleared_values, dash.no_update
            new_val = set_values[val_index]
            if new_val is None or new_val == '':
                return "Please enter a numeric value.", dash.no_update, cleared_values, dash.no_update
            try:
                res = reading_goals_backend.update_progress_manual(int(goal_id), int(new_val))
            except ValueError:
                return "Invalid numeric value.", dash.no_update, cleared_values, dash.no_update
            return res.get('message') if res.get('success') else f"Error: {res.get('message')}", \
                   {"last_action": "set", "goal_id": goal_id, "time": datetime.now().isoformat()}, cleared_values, dash.no_update

        if typ == 'delete-btn':
            res = reading_goals_backend.delete_goal(int(goal_id))
            return res.get('message') if res.get('success') else f"Error: {res.get('message')}", \
                   {"last_action": "delete", "goal_id": goal_id, "time": datetime.now().isoformat()}, cleared_values, dash.no_update
...
# Enable/disable date picker based on dropdown
@callback(
    Output('rg-end-date', 'disabled'),
    Input('rg-goal-type', 'value')
)
def toggle_end_date(goal_type):
