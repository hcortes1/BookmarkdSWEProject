...
def create_reading_goals_tab_content(user_data, is_own_profile):
    ...
    return html.Div([
        ...
        html.Div([
            html.Div([
                html.Div([
                    html.H3("Create New Reading Goal",
                ...
                html.Div([
                    ...
                    dcc.Checklist(
                        ...
                        options=[
                            {'label': ' Enable reminders for this goal', 'value': 'on'}],
                        value=[],
                        style={'margin-bottom': '20px'}
                    ),

                    html.Div(id='profile-rg-create-status', style={
                             'margin-bottom': '15px', 'text-align': 'center', 'font-weight': '600'}),
...
def create_reading_goal_card(goal, user_id):
    ...
    start_text = start_date.strftime(
        '%m/%d/%Y') if hasattr(start_date, 'strftime') else (start_date or 'Unknown')
    ...
    return html.Div([
        ...
        html.Div([
            ...
            html.Div([
                ...
                html.Span(end_text),
                html.Span(f" ({days_text})" if days_text else "", style={
                          'color': '#dc3545' if 'Overdue' in days_text else '#28a745', 'font-weight': '600', 'margin-left': '5px'})
            ], style={'margin-bottom': '8px'}),
            html.Div([
        ...
        html.Div([
            html.Label("Update Progress:", style={
                       'font-weight': '600', 'margin-bottom': '8px', 'display': 'block'}),
            html.Div([
                dcc.Input(
...
def update_tab_content(active_tab, session_data, viewed_username):
    ...
    if active_tab == 'profile':
        ...
        tab_content = create_reading_goals_tab_content(
...
def create_profile_info_card(user_data, is_own_profile, session_data):
    ...
    friend_request_section = html.Div(id='profile-friend-request-section')
    if is_own_profile and session_data and session_data.get('logged_in', False):
        ...
        if status == 'friends':
            ...
            friend_request_section.children = [html.Div([
                html.Span("This user sent you a friend request. Check your notifications!", className='text-link',
                          style={'font-weight': 'bold', 'margin-top': '0px', 'display': 'block'})
            ])]
        else:
        ...
        stylesheet=[
            ...
            },
            {
                ...
                'style': {
                    ...
                    'font-size': '10px',
                    'font-weight': 'bold',
                    'text-background-color': 'var(--card-bg)',
                    'text-background-opacity': 1,
...
@callback(
    Output("profile-friend-request-section", "children", allow_duplicate=True),
...
def create_reading_goal(n_clicks, session_data, goal_type, book_id, book_options, target, end_date, reminder_value, current_trigger):
    ...
    if success:
        ...
        return (
            html.Span("Goal created successfully!",
                      style={'color': '#28a745'}),
            current_trigger + 1,
            None,  # Clear target
            None,  # Clear book_id
...
def update_goal_progress(set_clicks, progress_values, progress_ids, current_trigger):
    ...
    val_index = next((i for i, pid in enumerate(progress_ids)
                     if pid['goal_id'] == goal_id), None)
    ...
    import backend.reading_goals as reading_goals_backend
    result = reading_goals_backend.update_progress_manual(
        int(goal_id), int(new_progress))
...
def refresh_reading_goals(trigger, session_data, viewed_username, active_tab):
    ...
    is_own_profile = (session_data.get('username', '').lower()
                      == viewed_username.lower())
